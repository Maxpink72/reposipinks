...
import {
  processResearchExportPdfJob,
  processResearchExportXlsxJob,
} from "@/modules/research/lib/export/process-export-job";
...
const RESEARCH_EXPORT_PDF_JOB_NAME = "research.export.pdf";
const RESEARCH_EXPORT_XLSX_JOB_NAME = "research.export.xlsx";
...
const researchExportXlsxJobHandler: NonNullable<JobHandlerOverrides[string]> = async (data, _context) => {
  const payload = data as TResearchExportXlsxJobData;
  await processResearchExportXlsxJob(payload.exportJobId);
};
...
export const registerJobsWorker = async (): Promise<JobsRuntimeHandle | null> => {
  ...
  const jobHandlerOverrides: JobHandlerOverrides = runtimeOptions.jobHandlerOverrides
    ? {
        ...
        [RESEARCH_EXPORT_PDF_JOB_NAME]: researchExportPdfJobHandler,
        [RESEARCH_EXPORT_XLSX_JOB_NAME]: researchExportXlsxJobHandler,
      }
    : {
        [RESPONSE_PIPELINE_JOB_NAME]: responsePipelineJobHandler,
        [SURVEY_SCHEDULING_JOB_NAME]: surveySchedulingJobHandler,
