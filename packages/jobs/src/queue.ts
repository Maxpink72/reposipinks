...
import {
  type TResearchExportPdfJobData,
  type TResearchExportXlsxJobData,
...
export const enqueueResearchExportPdfJob = async (data: TResearchExportPdfJobData): Promise<Job> => {
  try {
    return await enqueueBackgroundJob(JOB_NAMES.researchExportPdf, data);
  } catch (error) {
    logger.error(
      { err: error, jobName: JOB_NAMES.researchExportPdf },
      "Failed to enqueue BullMQ research export PDF job"
    );
    throw error;
  }
};

export const enqueueResearchExportXlsxJob = async (data: TResearchExportXlsxJobData): Promise<Job> => {
  try {
    return await enqueueBackgroundJob(JOB_NAMES.researchExportXlsx, data);
  } catch (error) {
    logger.error(
      { err: error, jobName: JOB_NAMES.researchExportXlsx },
      "Failed to enqueue BullMQ research export XLSX job"
    );
    throw error;
  }
};
...
export const getBackgroundJobProducer = (): BackgroundJobProducer => ({
  ...
  enqueueResearchExportPdf: async (data) => toEnqueuedJob(await enqueueResearchExportPdfJob(data)),
  enqueueResearchExportXlsx: async (data) => toEnqueuedJob(await enqueueResearchExportXlsxJob(data)),
