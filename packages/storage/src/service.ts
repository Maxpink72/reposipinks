...
/**
 * Upload a file buffer directly to S3 (server-side PutObject).
 * Used by background export workers that cannot use browser presigned POST.
 */
export const putObject = async (params: {
  fileKey: string;
  body: Buffer | Uint8Array;
  contentType: string;
}): Promise<Result<{ fileKey: string; byteSize: number }, StorageError>> => {
  try {
    const s3Client = createS3Client();
    ...
    logger.error({ error, fileKey: params.fileKey }, "Failed to put object to S3");
    return err({
      code: StorageErrorCode.Unknown,
    });
  }
